...
class EnglishPhrases(BaseClass):
    ...
    def get_all_words_from_file(self) -> List:
        ...
        path_to_file: str = f'{self.project_path}/list_of_english_phrases.txt'
        if not os.path.exists(path_to_file):
            text: str = 'Отсутствует файл list_of_english_phrases.txt'
            notification: Notification = Notification(
                subject='english',
                title='English irregular verb to repeat',
                content=text
            )
        ...
        with open(path_to_file, encoding='utf-8') as file:
            for line in file:
                ...
                if re.findall(r'^-\s?[а-я|А-Я|\s|\,|\.|\-|\!|\(|\)]+?\:[a-z|A-Z|\s|\,|\.|\-|\!|\(|\)]+$', line):
                    ...
                    text: str = f'Фраза записано неправильно - {line}\n'
                    notification: Notification = Notification(
                        subject='english',
                        title='English irregular verb to repeat',
                        content=text
        ...
        if not all_words_from_file:
            text: str = f'Нет фраз для повторения'
            notification: Notification = Notification(
                subject='english',
                title='English irregular verb to repeat',
                content=text
